...
def get_wb_fmeasure(golden_lists, predict_lists, label_type="BMES"):

    prec = compute_precision_token_basis(predict_lists, golden_lists, 'A')
    rec = compute_precision_token_basis(golden_lists, predict_lists, 'A')

    sentence_accuracy = compute_sentence_accuracy(predict_lists, golden_lists)

    print('')
    print('')
    print('Sentence accuracy : %.4f' % sentence_accuracy)
    print('')
    print('')
...
def compute_sentence_accuracy(guessed_sentences, correct_sentences):
    assert (len(guessed_sentences) == len(correct_sentences))
    correctCount = 0
    count = 0
    ...
    for sentenceIdx in range(len(guessed_sentences)):
        ...
        guessed_tags = ''.join(guessed)
        correct_tags = ''.join(correct)

        count += 1
        if guessed_tags == correct_tags:
            correctCount += 1

    accuracy = float(correctCount) / count
